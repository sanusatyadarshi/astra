...
func TestRetryWorks(t *testing.T) {
	...
	mock := NewMockOperation(ctrl)
	mock.EXPECT().Do().Return("", errors.New("")).Times(2)
	mock.EXPECT().Do().Return("success", nil).Times(1)
	retryOperation(mock.Do)
	// Only verifies mock call count, not real behavior
}
```
...
```bash
go test ./path/to/package -run TestRetryOperation
```
...
func retryOperation[T any](fn func() (T, error), opts ...RetryOptions) (T, error) {
	// YAGNI
}
```
Over-engineered

...
- Empty string as `cmd.Dir` resolves to `os.Getwd()`
...
func gitInit(directory string) error {
	...
	cwd, _ := os.Getwd()
	log.Printf("DEBUG git init: dir=%s cwd=%s env=%s\nstack:\n%s",
		directory, cwd, os.Getenv("GO_ENV"), buf[:n])
...
**Capture stack:** `runtime.Stack()` shows complete call chain
